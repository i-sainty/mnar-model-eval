...
# ---------------------------------------------------------
# 3. Build the LightGBM Preprocessor (NO NUMERICAL IMPUTATION)
# ---------------------------------------------------------
...
lgb_cat_pipeline = Pipeline([
    ('imputer', SimpleImputer(strategy='most_frequent')), 
    ('encoder', OneHotEncoder(handle_unknown='ignore', sparse_output=False))
])

# Full Preprocessor: Pass raw NaNs straight to LightGBM for numerical columns
lgb_preprocessor = ColumnTransformer(
    transformers=[
        ('num', 'passthrough', numerical_cols), 
        ('cat', lgb_cat_pipeline, categorical_cols)
    ])
...
for rate in rates_to_test:
    ...
    # 2. Process data (NaNs remain in the numerical columns)
    X_train_processed = lgb_preprocessor.fit_transform(X_train_mnar)
    X_test_processed = lgb_preprocessor.transform(X_test) 
...
print("\n🎉 LIGHTGBM EXPERIMENTS COMPLETE! Check ../reports/experiment_log.csv")
